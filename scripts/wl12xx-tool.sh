...
prepare_reference_nvs_file()
{
	...
	if [ "$1" -eq "2" ]; then
		echo -e "+++ Create reference NVS with " \
"$path_to_ini $path_to_ini2"
		ret=`"$path_to_calib"calibrator set ref_nvs2 $path_to_ini $path_to_ini2`
		if [ "$ret" == "" ]; then
...
cmd_tx_bip()
{
	...
	if [ "$run_it" != "" ]; then
		"$path_to_calib"calibrator wlan0 plt tx_bip 1 1 1 1 1 1 1 1 \
$nvs_full_path
		if [ "$?" != "0" ]; then
		...
		"$path_to_calib"calibrator wlan0 plt tx_bip 1 0 0 0 0 0 0 0 \
$nvs_full_path
		if [ "$?" != "0" ]; then
			echo -e "Fail to calibrate"
			return 1
